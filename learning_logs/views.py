...
from .forms import PostForm, HeadlineForm
...
# @login_required
def topics(request):
    topics = Headline.objects.filter().order_by('date_added')
...
def topic(request, topic_id):
    ...
    topic = Headline.objects.get(id=topic_id)
    # Make sure the topic belongs to the current user
    # if topic.owner != request.user:
    #     raise Http404
    entries = topic.post_set.order_by('-date_added')
